	...
	for(int i=0; i<34; i++){
		if (!chi[0][i].empty()){
			for (auto j=chi[0][i].begin(); j!=chi[0][i].end(); j++){
				++ht.pack_count;
				ht.pack_t[ht.pack_count]=make_pack(*j,1,maketile(i/9+1,i%9));
			}
		}
		if (peng[0][i]!=0){
			++ht.pack_count;
			ht.pack_t[ht.pack_count]=make_pack(peng[0][i],2,maketile(i/9+1,i%9));
		}
		if (gang[0][j]!=0){
			++ht.pack_count;
			ht.pack_t[ht.pack_count]=make_pack(gang[0][i],3,maketile(i/9+1,i%9));
		}
		if (angang[i]){
			++ht.pack_count;
			ht.pack_t[ht.pack_count]=make_pack(0,3,make_pack(i/9+1,i%9));
		}
...
bool enum_callback(shant* context,majong::enum_result_t* result){
	...
	if (result->shanten==-1) return false;
}
shant* context;
context->shanten=13;
