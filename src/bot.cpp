...
//    把string表示的手牌转化为存储数字
    if(s[0]=='W')return s[1]-'1';
    if(s[0]=='B')return 9+s[1]-'1';
    if(s[0]=='T')return 18+s[1]-'1';
    if(s[0]=='F')return 27+s[1]-'1';
    if(s[0]=='J')return 31+s[1]-'1';
    return 34;
}
...
struct info{
    ...
    bool canMingGang(int tile){
        //return是否可杠
        if(hand[tile]==3)return 1;
        return 0;
    }
    
    bool canPeng(int tile){
        //return是否可碰
        if(hand[tile]==2)return 1;
        return 0;
    }
    ...
    int canHu(int winTile,bool isZIMO,bool isGANG){
        ...
        vector<pair<int, string> > h=MahjongFanCalculator(pack(),hand_num2str(),tile_num2str(winTile),
        hua[myPlayerID],isZIMO,isJUEZHANG(winTile),isGANG,leftTile==0,myPlayerID,quan);
        int totalFan=0;
        for (auto i=h.begin();i!=h.end();++i){
            totalFan+=i->fr;
        }
        if(totalFan>=8)return totalFan;
        else return 0;
    }
        ...
        for(int i=0;i<4;++i){
            for(int j=0;j<34;++j){
                ...
                for(int k=0;k<tmp[i][j].size();++k)
                    vtmp.push_back(tmp[i][j][k].asInt();)
                chi[i][j]=tmp[i][j].asInt();
            }
        }
        tmp = root["play"];
        for(int i=0;i<4;++i){
        ...
        if(stmp=="BUHUA"){
            ...
            if(info.canMingGang(t))
                ...
                if(!v.empty()){
                    ...
                    for(int i=33;i>=0;--i){
                        if(i!=v.front()&&i!=v.front()+1&&i!=v.front()-1 && info.hand[i]!=0)
                            resp+=tile_num2str(i);
                    }
