...
int checkHu(int player, bool finish) {
    ...
    for (unsigned int i = 0; i < playerData[player].tile.size(); i++) {
        calculate_param.hand_tiles.standing_tiles[i] =
            str2tile[playerData[player].tile[i]];
    }
    ...
    for (unsigned int i = 0; i < playerData[player].pack.size(); i++) {
        ...
        if (sPack.type == "PENG") {
            dPack = mahjong::make_pack((sPack.offer - player + 4) % 4,
                                       PACK_TYPE_PUNG, str2tile[sPack.tile]);
        } else if (sPack.type == "GANG") {
            dPack = mahjong::make_pack((sPack.offer - player + 4) % 4,
                                       PACK_TYPE_KONG, str2tile[sPack.tile]);
        } else {
            dPack = mahjong::make_pack(sPack.offer + 1, PACK_TYPE_CHOW,
                                       str2tile[sPack.tile]);
        }
    }
    ...
    if (outputList.size() == 1) {
        ...
        if (outputList[0] == "PLAY") {
            vector<string>::iterator curr =
                find(playerData[player].tile.begin(),
                     playerData[player].tile.end(), lastTile);
            ...
            playerData[player].pack.push_back(
                PlayerData::Pack("GANG", lastTile, player));
            lastOp = "GANG";
            currANGANG = true;
            ...
            for (unsigned int i = 0; i < playerData[player].pack.size(); i++) {
                if (playerData[player].pack[i].type == "PENG" &&
                    ...
                    auto it = find(playerData[player].tile.begin(),
                                   playerData[player].tile.end(), lastTile);
...
bool checkInputPLAY2(const Json::Value &playerOutput, int player) {
    ...
    if (outputList.size() == 1) {
        if (outputList[0] == "PASS") {
            ...
            playerData[player].pack.push_back(
                PlayerData::Pack("GANG", lastTile, roundStage % 4));
            roundStage = player + 8;
            return true;
        }
        playerError(player, "WA");
        ...
        if (outputList[0] == "PENG") {
            ...
            lastOp = "PENG";
            playerData[player].pack.push_back(
                PlayerData::Pack("PENG", lastTile, roundStage % 4));
            lastTile = outputList[1];
            vector<string>::iterator curr =
                find(playerData[player].tile.begin(),
                     playerData[player].tile.end(), lastTile);
...
bool checkInputPLAY3(const Json::Value &playerOutput, int player) {
    ...
    if (outputList.size() == 3) {
        ...
        playerData[player].pack.push_back(PlayerData::Pack(
            "CHI", tileCHI, lastTile[1] - outputList[1][1] + 1));
        lastTile = outputList[2];
        vector<string>::iterator curr =
            find(playerData[player].tile.begin(), playerData[player].tile.end(),
                 lastTile);
        if (curr == playerData[player].tile.end()) {
            playerError(player, "WA");
        }
...
void checkInputGANG(const Json::Value &playerOutput, int player) {
    ...
    if (lastBUGANG && roundStage % 4 != player &&
        playerOutput["response"] == "HU") {
        checkHu(player, true);
    }
    playerError(player, "WA");
}
...
void roundOutput(Json::Value &outputValue) {
    if (roundStage == -1) {
        ...
        if (lastTile[0] == 'H') {
            ...
            for (int i = 0; i < 4; i++) {
                outputValue["content"][to_string(i)] =
                    "3 " + to_string(roundStage) + " BUHUA " + lastTile;
            }
        } else {
            lastOp = "DRAW";
            ...
            for (int i = 0; i < 4; i++) {
                if (roundStage % 4 == i) {
                    ...
                    outputValue["content"][to_string(i)] =
                        "3 " + to_string(roundStage % 4) + " DRAW";
                }
            }
            outputValue["display"]["canHu"][roundStage] =
        ...
        for (int i = 0; i < 4; i++) {
            if (lastOp == "CHI") {
                outputValue["content"][to_string(i)] =
                    "3 " + to_string(roundStage % 4) + " CHI " + tileCHI + " " +
                    lastTile;
            } else {
                outputValue["content"][to_string(i)] =
                    "3 " + to_string(roundStage % 4) + " " + lastOp + " " +
                    lastTile;
            }
...
int main() {
    ...
    for (int i = 1; i <= 4; i++) {
        str2tile["F" + to_string((i))] =
            mahjong::make_tile(TILE_SUIT_HONORS, i);
    }
    for (int i = 1; i <= 3; i++) {
        str2tile["J" + to_string((i))] =
    ...
    // "content": {
    //             "0": "1 1 0 1 2 W3 T5 T1 T7 T7 J1 T2 F2 T4 W6 F2 B6 T5 H3 H1
    //             H7 H5", "1": "1 1 0 1 2 T9 W9 J1 B3 W7 W7 T3 B3 T9 W1 T7 W8
    //             T5 H3 H1 H7 H5", "2": "1 1 0 1 2 W3 T2 B5 F1 W2 W7 F3 B7 F3
    //             W4 B5 T3 W8 H3 H1 H7 H5", "3": "1 1 0 1 2 B4 W3 B7 T4 W5 B1
    //             W5 B3 J2 T3 F1 F4 W9 H3 H1 H7 H5"
    //         }
